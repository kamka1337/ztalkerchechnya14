...
public sealed class KuklaExamineSystem : EntitySystem
{
    ...
    private void OnExamineTooltipBuilt(EntityUid player, EntityUid target, BoxContainer vBox, bool healthTab)
    {
        ...
        var view = new KuklaView { VerticalAlignment = Control.VAlignment.Center };
    ...
    private FormattedMessage BuildStatus(LimbHealthComponent comp)
    {
        ...
        foreach (var limb in Order)
        {
            ...
            string? cond = null;
            if (st.Destroyed)
                cond = $"[color=#bbbbbb]{DestroyedWord(limb)}[/color]";
            else if ((st.Effects & LimbEffect.HeavyBleeding) != 0)
            ...
            else if ((st.Effects & LimbEffect.Fracture) != 0)
                cond = "[color=#ffff66]перелом[/color]";

            if (cond == null)
                continue;

            any = true;
            sb.Append($"{LimbName(limb)}: {cond}\n");
        }

        if (!any)
            sb.Append("[color=#88dd88]повреждений нет[/color]");
